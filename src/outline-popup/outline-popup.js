...
(function outlinePopup() {
  ...
  // Close popup globally (safe even if not built yet)
  function closePopup() {
  ...
  // Focus a heading element after scroll, with a temporary tabindex
  ...
  function isPopupOpen() {
    return !!state.popup.classList.contains('open');
  }
  ...
  function buildOutlineContainers() {
    ...
    rail.addEventListener('mouseenter', () => { state.railHover = true; schedulePopupShow(); });
    rail.addEventListener('mouseleave', () => { state.railHover = false; schedulePopupHide(); });
    popup.addEventListener('mouseenter', () => { state.popupHover = true; schedulePopupShow(); });
    popup.addEventListener('mouseleave', () => { state.popupHover = false; schedulePopupHide(); });
    ...
    // On mobile devices, tapping the rail opens popup
    if (cfg.mobilePopup) rail.addEventListener('click', openPopup);
  ...
  function buildRailAndPopup() {
    ...
    // Bars (not keyboard-focusable; clickable only)
    state.headings.forEach((h, i) => {
      ...
      // enable click to scroll on non-touch devices only (on touch, rail click opens popup)
      if (!cfg.mobilePopup) bar.addEventListener('click', (e) => { e.preventDefault(); scrollToIndex(i); });
